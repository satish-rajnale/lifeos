...
export const VoiceCaptureModal = ({ visible, onClose, onCapture }: VoiceCaptureModalProps) => {
    ...
    useEffect(() => {
        if (visible) {
            logger.info('VOICE_START', { 
                source: 'VoiceCaptureModal',
                message: 'Modal opened, starting listening in 300ms'
            })
            ...
            logger.info('VOICE_STOP', { 
                source: 'VoiceCaptureModal',
                message: 'Modal closed, stopping listening'
            })
            stopListening()
    ...
    const handleDone = async () => {
        logger.info('VOICE_STOP', { 
            source: 'VoiceCaptureModal',
            message: 'User pressed done button',
            hasTranscript: !!transcript,
        ...
        if (!transcript) {
            logger.warn('VOICE_STOP', { 
                source: 'VoiceCaptureModal',
                message: 'No transcript captured, closing modal'
            })
            onClose()
            return
        }
